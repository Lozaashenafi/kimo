...
__export(index_exports, {
  ...
  getEmoji: () => getEmoji,
  getEmojiMetadata: () => getEmojiMetadata,
  getEmojiSvg: () => getEmojiSvg,
  getEmojisByCategory: () => getEmojisByCategory,
  listEmojiNames: () => listEmojiNames,
...
function getEmojiMetadata(name) {
...
function listEmojiNames() {
  return [...EMOJI_NAMES];
}
function searchEmojis(query, options = {}) {
  ...
  let results = EMOJI_LIST.filter((emoji) => {
    ...
    if (exact) {
      return nameMatch === normalizedQuery || labelMatch === normalizedQuery || emoji.keywords.some((k) => k.toLowerCase() === normalizedQuery);
    }
...
function getEmojiSvg(name, options = {}) {
  const emoji = getEmoji(name);
  if (!emoji) {
  ...
  svg = svg.replace(/<svg\b([^>]*)>/i, (_match, attrs) => {
    ...
    if (style) {
      const styleString = typeof style === "string" ? style : Object.entries(style).map(([k, v]) => `${k.replace(/([A-Z])/g, "-$1").toLowerCase()}:${v}`).join(";");
...
function createEmoji(name, options = {}) {
